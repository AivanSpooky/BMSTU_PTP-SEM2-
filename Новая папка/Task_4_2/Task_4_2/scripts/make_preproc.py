...
def analyze(time):
    ...
    median = time[len(time)//2] if (len(time)%2 == 1) else (time[len(time)//2] + time[len(time)//2+1]) // 2
...
for opt in options:
    for sort_type in sort_types:
        ...
        for size in sizes:
            ...
            extr = extra(time)
            if (extr > 5):
                print(f"For ./dataset/{opt}_s{sort_type}_n{size}.txt - rse > 5%")
            if (entered):
                try:
            ...
            f = open(f"./graph/{opt}_s{sort_type}_rse.txt", 'a')
            entered = True
            tmp_extr = extr
            tmp_size = size
